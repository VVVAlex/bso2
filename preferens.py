...
class Window:
    ...
    def __init__(self, master):
        ...
        # self.in_v = ctk.CTkEntry(self.frame, validate='key', font=font, state='disabled',
    ...
    def ok(self, arg=None) -> None:
        ...
        for i, j in enumerate(self.chosen):
            if _dt == j:
                _d = i
        if (_t, _h, _d, _zona) != (self.t, self.h, self.d, self.z):
            self.save_(_t, _h, _d, _zona)
            self.root.pref_form(_dt, _z, _zona)
        self.close()
    ...
    def save_(*arg) -> None:
        ...
        config.set('System', 'vzona', f'{arg[3]}')
        write_config()
