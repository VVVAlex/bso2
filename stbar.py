...
class Footer(ctk.CTkFrame):
    ...
    def __init__(self, root):
        ...
        self.im_teld = load_image('win_net2.png')
        self.im_tell = load_image('win_net.png')
        ...
        self.img_label = ctk.CTkLabel(self, textvariable=self.port_info,
    ...
    def blink(self):
        """Мигнуть телеком"""
        if self.root.theme_mode:
            im = self.im_tell
            im2 = self.im_teld
        else:
            im = self.im_teld
            im2 = self.im_tell
        self.img_label.configure(image=im)
        self.root.after(200, lambda: self.img_label.configure(image=im2))
